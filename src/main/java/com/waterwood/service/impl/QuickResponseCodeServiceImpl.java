...
public class QuickResponseCodeServiceImpl implements IQuickResponseCodeService {
	...
	public List<QuickResponseCode> generateQRCode(int count, String patchCode, String path, String realpath) {
		...
		for (int i = 0; i < count; i++) {
			...
			String insideCodeText = String.format(path + Global.URL_VALIDATE + "?code=" + qrcode.getQrcodeInsidecode());
	...
	public String createOutsideCode(String text, String realpath) {
		...
		try {
			...
			// 生成二维码
			String relativePath = QrCodeUtil.OUT_QRCODE_IMG + "/" + Util.directorySdf.format(new Date()) + "/"
					+ Util.nameSdf.format(new Date()) + QrCodeUtil.IMG_SUFFIX;
	...
	public String createInsideCode(String text, String realpath) {
		...
		try {
			...
			BitMatrix bitMatrix = new MultiFormatWriter().encode(text, BarcodeFormat.QR_CODE, width, height, hints);
			// 生成二维码
			String relativePath = QrCodeUtil.IN_QRCODE_IMG + "/" + Util.directorySdf.format(new Date()) + "/"
					+ Util.nameSdf.format(new Date()) + QrCodeUtil.IMG_SUFFIX;
			File outputFile = new File(realpath + relativePath);
			MatrixToImageWriter.writeToFile(bitMatrix, format, outputFile);
	 ...
	 * @param logoPic
	 */
	private void addLogo_QRCode(String qrPicStr, File qrPic, File logoPic, LogoConfig logoConfig) {
		try {
